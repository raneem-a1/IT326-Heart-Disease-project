...
### Classification goal:
The aim is to predict whether an individual is at high risk of ASCVD or not, based on their clinical attributes (age, blood pressure, cholesterol, HDL, smoking status, etc.). Since the dataset provides Risk as a continuous value, we will convert it into a binary label individuals above a chosen risk threshold will be labeled as 1 (high risk), and those below it will be labeled as 0 (low risk). The objective is to build a predictive model that accurately classifies individuals into these two groups.
